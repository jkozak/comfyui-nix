...
let
  ...
  # The wheels bundle CUDA 13.0 libraries, so no separate CUDA toolkit needed at runtime
...
lib.optionalAttrs useCuda {
  torch = final.buildPythonPackage {
    ...
    # The wheel bundles CUDA 13.0 and supports all GPU architectures
    passthru = {
      cudaSupport = true;
      rocmSupport = false;
      ...
      # Provide cudaPackages for packages that need it
...
# (sm_90 CUTLASS kernels with CUDA 13.0 consume ~3GB RAM each)
