...
- Python version: 3.12
- PyTorch: macOS uses pre-built wheels (2.5.1, pinned to work around MPS bugs on macOS 26); CUDA uses pre-built wheels from pytorch.org (cu130); ROCm uses pre-built wheels from pytorch.org (rocm7.1); Linux CPU uses nixpkgs
...
- CUDA: Pre-built wheels from pytorch.org with CUDA 13.0 runtime bundled (no separate toolkit needed); supports Pascal through Blackwell
